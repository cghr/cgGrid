...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    meta: {
      modules: 'angular.module("cgGrid", [<%= srcModules %>]);',
      tplmodules: 'angular.module("cgGrid.tpls", [<%= tplModules %>]);',
      all: 'angular.module("cgGrid", ["cgGrid.tpls", <%= srcModules %>]);',
      banner: ['/*',
        ' * <%= pkg.name %>',
        ' * <%= pkg.homepage %>\n',
  ...
  //Common cgGrid module containing all modules for src and templates
  ...
  function findModule(name) {
    ...
    var module = {
      name: name,
      moduleName: enquote('cgGrid.' + name),
    ...
    grunt.file.expand('src/' + name + '/*.js')
      ...
      .forEach(function(contents) {
        ...
        dependencies.split(',').forEach(function(dep) {
          if (dep.indexOf('cgGrid.') > -1) {
            var depName = dep.trim().replace('cgGrid.', '').replace(/['"]/g, '');
